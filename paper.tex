...
This is likely due to the fact that evaluating its performance would require either a collection of MIDI and audio pairs for which the correct alignment is already known (which does not exist) or manual audition and rating of the output of the systems (which is time-consuming).
...
$$
%p, q = \mathrm{arg}\min_{p, q} \sum_{i = 1}^{L} \|X[p[i]] - Y[q[i]]\|_2^2 + \Phi(p, q)
p, q = \mathrm{arg}\min_{p, q} \sum_{i = 1}^{L} D(X[p[i]], Y[q[i]]) + \Phi(p, q)
$$
where $D(X,Y)$ is a frame-wise distance measure (such as squared Euclidean distance), and $\Phi(p, q)$ imposes additional costs on the paths; one common choice is to allow $p$ and $q$ to increase by 0 or 1 at each step in $i$, but to incur a cost of $\phi$ each time a zero-increase occurs; 
...
%\end{cases}
%$$
%% I think it would be better to include monotonicity at the top, and to combine the description of allowable steps with the different costs of different steps, but this is OK.
...
% Peak structure distance is not L2, and does not map both to the same domain.
...
\end{description}
% g handles differences in the end, but not in the beginning...
...
r[n] = \sum_{k = 0}^{N - 1} \mathcal{N}(0, \sigma_t) \cos(j2\pi kn/N) \\
+ \mathcal{N}(0, \sigma_t) \sin(j2\pi kn/N) \quad \quad n \in \{1, \ldots, N\}
\end{multline*}
...
% Don't understand.  If we screw it up a lot, what's the correct answer?
...
%To find systems with comparably high performance, we performed a
A one-sample $t$-test was performed to determine how which systems gave alignment quality statistically equivalent to the best performance. Testing the per-pair scores rather than the mean error across all pairs gave better robustness to outliers.  Using a $p$-value of $0.05$ as the threshold returned 51 configurations of indistinguishable quaity.
...
As described in Section \ref{sec:dtw}, DTW provides a ``raw'' distance as the sum of distances between all aligned feature vectors.
...
In the experiments described above, we have found a system which can provide both high accuracy and a useful metric on synthetic data.
...
%In an ideal world, all MIDI and audio files would be paired correctly, and all MIDI transcriptions would be perfect. However, a
